...
export function assertVideoFile(name: string, type: string, size: number, max: number): string {
  ...
  if (!VIDEO_EXTENSIONS.has(ext)) throw new Error("Use an MP4, MOV, WebM, or MKV file.");
  if (type && !VIDEO_MIME.has(type)) throw new Error("That file does not look like a video.");
...
export function parseVideoUrl(raw: string): URL {
  ...
  if (url.protocol !== "https:" && url.protocol !== "http:") throw new Error("Only http and https links are accepted.");
  const host = url.hostname.toLowerCase();
  if (!YOUTUBE_HOSTS.has(host)) throw new Error("Only YouTube links are accepted in this version.");
  if (host === "youtu.be" || host === "www.youtu.be") {
    if (!/^\/[\w-]{6,}$/.test(url.pathname) || url.searchParams.has("list")) {
  ...
  if ((!id || id.length < 6) && !shorts && !embed && !live) throw new Error("That YouTube link has no video id.");
  return url;
}
...
export const clipPatchSchema = z.object({
  ...
  start: z.number().finite().min(0).max(60 * 60 * 6).optional(),
  end: z.number().finite().min(0.5).max(60 * 60 * 6).optional(),
  aspect: z.enum(ASPECTS).optional(),
  captionStyle: z.enum(CAPTION_STYLES).optional(),
  captionText: z.string().max(500).optional(),
  cues: z.array(z.object({
    start: z.number().finite().min(0).max(60 * 60 * 6),
    end: z.number().finite().min(0).max(60 * 60 * 6),
