...
export async function analyzeProject(projectId: string, targetSeconds: number): Promise<void> {
  ...
  try {
    ...
    if (!transcript.words.length && provider.name !== "mock") {
      updateProject(projectId, {
        warning: "No speech was detected, so these cuts have no captions. You can type a line before printing.",
      });
    }
    ...
    const clips: Clip[] = drafts.map((draft, index) => {
      ...
      const end = Math.min(project.duration, Math.max(draft.start + 3, draft.end));
    ...
    if (provider.name !== "mock") fs.rmSync(audioPath, { force: true });
    const current = getProject(projectId);
    const warning = [current?.warning, trackWarning ? `Speaker tracking failed, so the frame stays centered. ${trackWarning}` : null]
      .filter(Boolean)
      .join(" ");
    updateProject(projectId, {
      status: "ready",
      stage: clips.some((clip) => clip.cues.length) ? "Cuts are on the bench" : "Cuts are ready, without captions",
      progress: 100,
